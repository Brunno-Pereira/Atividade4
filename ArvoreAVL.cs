...
namespace ArvoreAVL
{
	class ArvoreAVL
	{
		...
		private No Inserir(No no, int x) {
			if (no == null) {											//Caso a arvore seja recem-formada
				no = new No(x);
				maisAlto = true;

			}else if (x < no.info) {
				no.noEsquerdo = Inserir(no.noEsquerdo, x);				//A insercao ja é feita levando em conta o balanceamento

				if (maisAlto == true)
					no = VerificarInsercaoSubarvoreEsquerda(no);

			}else if (x > no.info) {									//A insercao ja é feita levando em conta o balanceamento
				...
				Console.WriteLine(x +  " ja esta presente na arvore");	//Autoexplicativo
				maisAlto = false;
			}
		...
		private No VerificarInsercaoSubarvoreEsquerda(No no)	//Esta funcao retorna o balanceamento  propriamente dito
		{
			switch (no.fatorBalanceamento)						//Ela informa para qual lado a arvore balanca
			{
		...
		private No RotacionarDireita(No no)	//Esta funcao simplesmente troca valores entre  folhas, mantendo o  custo da arvore minimizado
		{									//É uma operacao  entre folhas,  sendo custos  para o resto da arvore
		...
		private No Remover(No no, int x)	//Esta funcao remove  uma folha e ja performa o balanceamento da  arvore
		{
			No ch, s;
